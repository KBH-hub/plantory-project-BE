...
class MemberDetailServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        memberService = new AuthServiceImpl(authMapper, bCryptPasswordEncoder);
    }
    ...
    void testIsDuplicateMembernameTrue() {
        when(authMapper.countByMembername("testUser")).thenReturn(1);
    ...
    void testIsDuplicateMembernameFalse() {
        when(authMapper.countByMembername("testUser")).thenReturn(0);
    ...
    void testIsDuplicateNicknameTrue() {
        when(authMapper.countByNickname("nick")).thenReturn(1);
    ...
    void testIsDuplicateNicknameFalse() {
        when(authMapper.countByNickname("nick")).thenReturn(0);
    ...
    void testSignUpDuplicateMembername() {
        ...
        when(authMapper.countByMembername("aaa")).thenReturn(1);
    ...
    void testSignUpDuplicateNickname() {
        ...
        when(authMapper.countByMembername("aaa")).thenReturn(0);
        when(authMapper.countByNickname("aaa")).thenReturn(1);
    ...
    void testLoginSuccess() {
        ...
        when(authMapper.selectByMembername("hongTree")).thenReturn(memberResponse);
...
//        assertNotNull(result);
    ...
    void testLoginBlocked() {
        ...
        when(authMapper.selectByMembername("aaa")).thenReturn(member);
...
//        assertTrue(exception.getMessage().contains("정지 해제까지"));
    ...
    void testLoginUserNotFound() {
        when(authMapper.selectByMembername("aaa")).thenReturn(null);
